...
class Instagram extends React.Component{
  ...
  getRecentPosts(){
    ...
    let posts = [];
    let url = 'https://www.instagram.com/katieroseeeee/';
    ...
    axios.get(url).then(res => {
      ...
      url = url + 'p/';
      for(i = 0; i < posts.length; i++ ){
        posts[i] ="https://www.instagram.com/p/" + posts[i] + "/?utm_source=ig_embed&utm_campaign=loading" ;
      }
  ...
  getPostsDiv(){
    ...
    if (this.state.posts_arr == null){
      return;
    }
        console.log(this.state.posts_arr);

        console.log("TEST: " + this.state.posts_arr[0])
  return(
    <div>
    <blockquote className="instagram-media" data-instgrm-captioned data-instgrm-permalink={this.state.posts_arr[0]} data-instgrm-version={12} style={{background: '#FFF', border: 0, borderRadius: '3px', boxShadow: '0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15)', margin: '1px', maxWidth: '540px', minWidth: '326px', padding: 0, width: 'calc(100% - 2px)'}}>
          <div style={{padding: '16px'}}>
            <a href={this.state.posts_arr[0]} style={{background: '#FFFFFF', lineHeight: 0, padding: '0 0', textAlign: 'center', textDecoration: 'none', width: '100%'}} target="_blank">
            ...
            <p style={{margin: '8px 0 0 0', padding: '0 4px'}}> <a href={this.state.posts_arr[0]} style={{color: '#000', fontFamily: 'Arial,sans-serif', fontSize: '14px', fontStyle: 'normal', fontWeight: 'normal', lineHeight: '17px', textDecoration: 'none', wordWrap: 'break-word'}} target="_blank">so close yet so far @donovans_reef</a></p>
            <p style={{color: '#c9c8cd', fontFamily: 'Arial,sans-serif', fontSize: '14px', lineHeight: '17px', marginBottom: 0, marginTop: '8px', overflow: 'hidden', padding: '8px 0 7px', textAlign: 'center', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>A post shared by <a href="https://www.instagram.com/katieroseeeee/?utm_source=ig_embed&utm_campaign=loading" style={{color: '#c9c8cd', fontFamily: 'Arial,sans-serif', fontSize: '14px', fontStyle: 'normal', fontWeight: 'normal', lineHeight: '17px'}} target="_blank"> Katie Rose</a> (@katieroseeeee) on <time style={{fontFamily: 'Arial,sans-serif', fontSize: '14px', lineHeight: '17px'}} dateTime="2020-06-12T22:07:11+00:00">Jun 12, 2020 at 3:07pm PDT</time></p>
          </div>
        </blockquote>
    </div>
  );
